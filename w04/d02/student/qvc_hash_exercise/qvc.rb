...
def item_margins
  # $items.map { |id,item_hash| item_hash[("sell amount".to_i)-("manufacturer cost".to_i)/"sell amount".to_i]}
  $items.map do |id, item|
    profit = item["sell amount"] - item["manufacturer cost"]
    estimate = profit / item["sell amount"]
    estimate.round(2)
  end
end
